...
async fn auth_handler(
    ...
    if req.method() == lambda_http::http::method::Method::OPTIONS {
        ...
        match req.uri().path() {
            ...
            "/auth/authorize/patreon" => {
                authorize_patreon::login_patreon_handler(req, ctx, app_ctx)
            }
            "/auth/authorize/patreon/callback" => {
                patreon_callback::patreon_callback_handler(req, ctx, app_ctx).await
            }
